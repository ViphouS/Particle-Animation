...
export default class Sketch {
  constructor(options) {
    ...
    window.addEventListener("scroll", event => {
      // Model
      if (this.dna) {
        const scrollAmount = window.scrollY - previousScrollY;
        const scrollableEnd = document.documentElement.scrollHeight - window.innerHeight;
    
        if ((scrollAmount > 0 && window.scrollY >= scrollableEnd) || (scrollAmount < 0 && window.scrollY <= 0)) {
          event.preventDefault(); // Disable scrolling when at the top or bottom of the page
        } else { 
          this.dna.rotation.y += scrollAmount * 0.002;
          this.dna.position.y += scrollAmount * 0.002;
        }
